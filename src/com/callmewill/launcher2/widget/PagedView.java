...
public abstract class PagedView extends ViewGroup implements
	...
	public boolean onInterceptTouchEvent(MotionEvent ev) {
		 ...
		 * 最常见的情况是：用户处于拖动的状态下，同时在移动它的手指，这时候我们需要拦截这个动作。
		 */
		...
		case MotionEvent.ACTION_MOVE: {
			 ...
			 * original down touch.
			 * 如果mIsBeingDragged==false ,否则快捷方式应该捕获到该事件，
			 * 检查一下用户从它点击的地方位移是否足够 
			 */
			if (mActivePointerId != INVALID_POINTER) {
				//根据移动的距离判断是翻页还是移动一段位移，
				//同时设置lastMotionX或者mTouchState这些值。同时取消桌面长按事件
...
        	 * 在滑动的isFinished将返回false. 
		...
		case MotionEvent.ACTION_MOVE:
			if (mTouchState == TOUCH_STATE_SCROLLING) {
				...
				//如果我们移动了一小段距离，我们则移动和更新mLastMotionX 。
				//我们保存Remainder变量是因为会检测我们 
				//是否是从最后的滚动点位移动的。
